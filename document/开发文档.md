...
- `getDslResp = () => {...}`
  ...
  如果发送框中没有内容，则直接返回。如果发送框中有内容，先用发送框中的内容替换所有`this.state.varList`中值为`"_text_"`的变量，然后将发送框中的内容添加到`this.state.msgList`中，然后将该消息和当前的状态发送到后端请求应答，之后调用`handleResponse` 处理后端返回的应答。
