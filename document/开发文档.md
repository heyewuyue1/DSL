...
- `Robot.mutex:Lock`
  ...
  互斥锁，用于并发时互斥访问`Robot.user_list`

- `Robot.expire_time:int `

  一次会话的有效时长（从建立连接到对话结束）
...
- `@app.get("/token")`
  ...
  在每次有新用户请求token的时候，会先检查目前的`robot.user_list`中有没有已经超时的用户，如果有的话，先把它们的信息删除，然后再为新用户构造token并返回。
